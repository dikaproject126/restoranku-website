    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Daftar Menu</h3>
                <p class="text-subtitle text-muted">Berbagai Menu Restoranku dan Terbaik.</p>
            </div>
                        ...
                        @foreach ($items as $item)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if ($item->is_active == 1)
                                        <form action="{{ route('item.updateStatus', $item->id) }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="is_active" value="0">
                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah anda yakin menonaktifkan menu ini?')">
                                    ...
                                    @else
                                        <form action="{{ route('item.updateStatus', $item->id) }}" method="POST">
                                            ...
                                            <button type="submit" class="btn btn-success btn-sm" onclick="return confirm('Apakah anda yakin aktifkan menu ini?')">
                                                <i class="bi bi-x-circle"></i> Aktifkan
                                            </button>
                                        </form>
                                    @endif
